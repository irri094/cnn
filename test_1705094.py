...
def read_image(image_path):
    ...
    for i,path in enumerate(image_path):
        ...
        X.append(img)
        names.append(path.split(sep=os.sep)[-1])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 test_1705094.py <directory>")
        exit(1)

    directory = sys.argv[1]
    d_path = glob.glob(directory+"/*.png")

    cnn = pickle.load(open("1705094_model.pkl", "rb"))

    dataD, names = read_image(d_path)
    X_test_all = dataD
    ...
    # create csv file in directory with columns Filename and Digit
    # write the names and predictions in the csv file
    print("writing to csv file")
    
    f = open(directory+"/1705094_prediction.csv", "w")
    f.write("Filename,Digit\n")

    for i in range(sz):
        j = cnn.predict(X_test_all[i])
        f.write(names[i]+","+str(j)+"\n")
